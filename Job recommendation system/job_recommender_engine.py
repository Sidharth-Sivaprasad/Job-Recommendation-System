...
def build_annoy_index(embeddings, num_trees=10):
    ...
    index = AnnoyIndex(dim, 'angular')  # Angular distance ~ cosine similarity
...
# Retrieve Top-K Jobs using Annoy
def retrieve_top_k_annoy(user_embedding, annoy_index, k=50):
    indices, distances = annoy_index.get_nns_by_vector(user_embedding[0], k, include_distances=True)
...
def recommend_jobs(df, user_input, model, job_embeddings, annoy_index):
    user_text = build_user_text(user_input)
    user_embedding = model.encode([user_text], normalize_embeddings=True)
    top_indices, _ = retrieve_top_k_annoy(user_embedding, annoy_index, k=50)
...
if __name__ == '__main__':
    ...
    annoy_index = build_annoy_index(job_embeddings, num_trees=10)

    while True:
        print("\n--- New User Recommendation Session ---")
        user_input = get_user_input()
        recommendations = recommend_jobs(df, user_input, model, job_embeddings, annoy_index)
